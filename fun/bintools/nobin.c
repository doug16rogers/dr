...
int keep_bin(unsigned char c) {
    return (c != 0) && (c < 0x7F);
}

size_t nobin_to_stdout(FILE* f) {
...
int main(int argc, char* argv[]) {
    ...
    size_t bytes_out = 0;
    if (argc == 1) {
        bytes_out += nobin_to_stdout(stdin);
    } else {
        int i = 0;
        for (i = 1; i < argc; i++) {
            ...
            if (NULL == f) {
                ...
                bytes_out += nobin_to_stdout(f);
                fclose(f);
            }
        }
    }
    fprintf(stderr, "%s: wrote %lu bytes.\n", PROGRAM, (unsigned long) bytes_out);
    return return_code;
}   /* main() */
